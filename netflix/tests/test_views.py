...
class TestMovieViewSet(TestCase):
    ...
    def test_get_list_movie(self):
        ...
        self.assertEquals(data[0]['name'], "Test1 movie name")
        self.assertEquals(data[0]['year'], "2022-12-12")
        self.assertEquals(data[0]['imdb'], 1)
        self.assertEquals(data[0]['genre'], "Drama")
        self.assertEquals(data[0]['actors'], [])
    ...
    def test_ordering(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertEquals(data[0]['imdb'], 5)
        print(f"**********{data}")
